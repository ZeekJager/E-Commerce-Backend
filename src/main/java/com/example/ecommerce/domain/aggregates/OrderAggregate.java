...
public class OrderAggregate {
    ...
    public Money calculateTotalAmount() {
        double total = products.stream()
                .mapToDouble(p -> p.getPrice().amount() * p.getStock().value())
                .sum();
    ...
    public void reduceStock() {
        for (Product product : products) {
            int newStock = product.getStock().value() - 1; // simplistic: one unit per product
            product.updateStock(new Quantity(newStock));
        }
    }
